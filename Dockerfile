...
# Base image: NZBGet v26.2 (LinuxServer build ls249)
FROM ghcr.io/linuxserver/nzbget:v26.2-ls249
...
ENV PRIVOXY_SKIP_FILE_SETUP=no
...
ENV NZBGET_S1_CONN=
ENV NZBGET_S1_SSL=
ENV NZBGET_S1_LEVEL=0
ENV NZBGET_S1_ENABLED=yes

# Monitoring and Auto-restart ENV
ENV ENABLE_MONITORING=yes
ENV MONITORING_PORT=8080
ENV MONITORING_LOG_LEVEL=INFO
ENV ENABLE_AUTO_RESTART=false
ENV RESTART_COOLDOWN_SECONDS=300
ENV MAX_RESTART_ATTEMPTS=3
ENV EXIT_ON_MAX_RESTARTS=true
ENV RESTART_ON_VPN_FAILURE=true
ENV RESTART_ON_NZBGET_FAILURE=true
ENV DISABLE_IP_LEAK_CHECK=false
ENV VPN_NETWORK=
...
LABEL org.opencontainers.image.title="nzbgetvpn" \
      ...
      org.opencontainers.image.base.name="ghcr.io/linuxserver/nzbget:v26.2-ls249"
